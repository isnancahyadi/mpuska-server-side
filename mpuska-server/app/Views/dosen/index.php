                        ...
                        <?php foreach (json_decode($dosen) as $key => $value) : ?>
                            <tr>
                                ...
                                <td class="text-center" style="width: 15%">
                                    <a href="<?= site_url('dosen/edit/' . $value->niy) ?>" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i></a>
                                    <form action="<?= site_url('dosen/delete/' . $value->niy) ?>" method="POST" class="d-inline" id="del-<?= $value->niy ?>">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button class="btn btn-danger btn-sm" data-confirm="Hapus Data?|Apakah Anda yakin?" data-confirm-yes="submitDel(<?= $value->niy ?>)">
